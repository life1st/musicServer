...
class AlbumModel {
    ...
    async updateAlbum({ albumInfo, musicId }: {
        ...
        if (existAlbum) {
            if (existAlbum.musicIds.length > LIMIT && musicId) {
                return hasUpdate
            }
            const { coverId: existCoverId, name: existName } = existAlbum
            const curAlbumInfo = { ...existAlbum, ...albumInfo }
            if (musicId) {
                curAlbumInfo.musicIds = Array.from(new Set(existAlbum.musicIds.concat(musicId)))
            }
            if (!existCoverId) {
